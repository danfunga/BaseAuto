...
class BaseballAutoPlayer{
    ...
    static RESTART_COUNTS_KEY_MAP:={}
    static RESTART_COUNTS_TITLE_KEY_MAP:={}
    ...
    setRestartCountResult(modeName, value){
        global baseballAutoGui, baseballAutoConfig
        if ( value ="" ){
            value:=0
        }
        this.countPerRestart[modeName]:=value

        baseballAutoConfig.saveRestartCount(player,modeName,value)
        baseballAutoGui.updateStatus( BaseballAutoPlayer.RESTART_COUNTS_KEY_MAP[modeName], value)
    }
    setResult( result ){
        global baseballAutoGui, baseballAutoConfig
        if ( result ="" ){
    ...
    addRestartCount( modeName ){        
        this.setRestartCountResult(modeName, this.countPerRestart[modeName]+1)         
    }
